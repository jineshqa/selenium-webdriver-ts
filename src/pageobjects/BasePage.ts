...
async function waitForLocated(driver, locator, maxRetries = 3) {
    ...
    do {
        ...
        throw new Error(`Still not able to locate element ${locator.toString()} after maximum retries (${retry}), Error message: ${error.message.toString()}`);
}
...
async function waitForVisible(driver, locator, maxRetries = 3) {
    ...
    do {
        ...
        throw new Error(`Element ${locator.toString()} still not visible after maximum retries (${retry}), Error message: ${error.message.toString()}`);
}
...
async sendKeys(locator, keys, retries = 1) {
  ...
  async click(locator, retries = 10) {
    try {
      ...
      if (retries === 0) {
        throw new Error(`Still not able to click ${locator.toString()} after maximum retries (${retries}), Error message: ${error.message.toString()}`);
      }
      await this.driver.sleep(250);
